...
public class CommentController {
    ...
    public ResponseEntity<Response> createCommment(Long blogid,String commentContent){
        try {
            blogService.createComment(blogid,commentContent);
        }catch (ConstraintViolationException e) {
            return  ResponseEntity.ok(new Response(false, ConstraintViolationExceptionHandler.getMessage(e)));
        }
        catch (Exception e) {
            return  ResponseEntity.ok(new Response(false,e.getMessage()));//返回该异常描述的字符串
        }
        return  ResponseEntity.ok(new Response(true,"处理成功","null"));
    }

    @DeleteMapping("{/id}")
    public ResponseEntity<Response> delectBlog(@PathVariable("id") Long id,Long blogId){
